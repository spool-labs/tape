...
impl Rpc for LiteSvmRpc {
    ...
    async fn get_block(&self, slot: u64) -> Result<tape_blocks::wire::Block, RpcError> {
        ...
        data.to_ui_confirmed_block()
            .and_then(tape_blocks::wire::Block::try_from)
    ...
    async fn send_and_confirm_transaction(
        ...
        // LiteSVM executes immediately, so send == send_and_confirm, and
        // commitment/preflight are meaningless against the in-memory bank.

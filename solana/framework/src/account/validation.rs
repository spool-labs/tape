...
/// Validation methods for AccountInfo.
pub trait AccountInfoValidation {
    ...
    /// Assert account is PDA with given seeds.
...
/// Trait for deserializing account data with owner and discriminator validation.
pub trait AsAccount {
    /// Load account as immutable typed reference.
...
/// Generic validation trait for account data types.
pub trait AccountValidation {
    /// Assert condition on account data.
    ...
    /// Assert condition on mutable account data.
    fn assert_mut<F>(&mut self, condition: F) -> Result<&mut Self, ProgramError>
    where
        F: Fn(&Self) -> bool;

    /// Assert condition on mutable with custom error.
    fn assert_mut_err<F>(
        &mut self,
        condition: F,
        err: ProgramError,
    ...
    /// Assert condition on mutable with custom message.

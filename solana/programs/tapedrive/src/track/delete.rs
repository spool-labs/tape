...
mod tests {
    ...
    // deleting an invalidated track does not refund capacity a second time
    #[test]
    fn delete_invalidated_track() {
        let fee_payer = Pubkey::new_unique();
        let authority = Pubkey::new_unique();
        let bucket_hash = Hash::new_unique();
        ...
        let (tape_address, _) = tape_pda(authority.into());
        let track_number = TrackNumber(0);
        let live_size = StorageUnits::mb(100);
        let track = CompressedTrack {
            tape: tape_address,
            key: bucket_hash,
            track_number,
        ...
        // Used covers other live tracks only; this track was refunded at invalidation.
        let tape = Tape {
            authority: authority.into(),
            capacity: StorageUnits::mb(1000),
            used: live_size,

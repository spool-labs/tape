...
pub fn process_vote_snapshot(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    let args = VoteSnapshot::try_from_bytes(data)?;
    let [
        fee_payer_info,
        system_info,
        voting_epoch_info,
    ...
    if vote.kind != VoteKind::Snapshot as u64
        || vote.hash != args.hash

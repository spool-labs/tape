...
pub fn process_propose_assignment(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    let args = ProposeAssignment::try_from_bytes(data)?;
    let [
        fee_payer_info,
        system_info,
        voting_epoch_info,
    ...
    let (vote_address, bump) = assignment_vote_pda(voting_epoch_id, target_epoch_id, args.hash);
    vote_info
        .is_empty()?
        .is_writable()?
    ...
    vote.kind = VoteKind::Assignment as u64;
    vote.hash = args.hash;
...
mod tests {
    ...
    fn propose_assignment() {
        ...
        let instruction =
            build_propose_assignment_ix(fee_payer.into(), voting_epoch_id, hash);

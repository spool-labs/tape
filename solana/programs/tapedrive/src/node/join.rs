...
pub fn process_join_committee(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    ...
    // A node evicted from (or suspended through) the target epoch cannot join it.
    if next <= node.suspended_until {
        return Err(TapeError::NodeSuspended.into());
    }
...
mod tests {
    ...
    // a node suspended through the target epoch cannot re-join it
    #[test]
    fn join_rejected_when_suspended() {
        let fee_payer = Pubkey::new_unique();
        let authority = Pubkey::new_unique();
        let committee_size: u64 = 128;
        let peer_capacity: u64 = committee_size * 3;
        ...
        // Suspended through the target epoch: next <= suspended_until.
        let node = Node {
            authority: authority.into(),
            pool: StakingPool {
                stake: TAPE(1_000),
        ...
        env.process_instruction(
            &instruction,
            &accounts,
            &[Check::err(TapeError::NodeSuspended.into())],
        );
    }

    // once the suspension expires (below the target epoch) the node may re-join
    #[test]
    fn join_after_cooldown() {
        let fee_payer = Pubkey::new_unique();
        let authority = Pubkey::new_unique();
        let committee_size: u64 = 128;
        let peer_capacity: u64 = committee_size * 3;
        ...
        // Suspension expired below the target epoch: next > suspended_until.
        let node = Node {
            authority: authority.into(),
            pool: StakingPool {
                stake: TAPE(1_000),
        ...
        // The node is appended to the next committee (lowest stake, sorts last).
        let joined = Member {
            node: node_address,
            stake: TAPE(1_000),
            assigned: StorageUnits::zero(),
        ...
        let expected_next_members = [member(3, 3_500), member(4, 2_100), joined];
        let expected_next_committee =
            Committee { epoch: next, members: Tail::new(committee_size, expected_next_members.len() as u64) }
                .pack_with(&expected_next_members);
        ...
        env.process_instruction(
            ...
            &[
                ...
                Check::account(&Pubkey::from(node_address))
                    .data(node.pack().as_ref())
                    .build(),
                Check::account(&Pubkey::from(next_committee_addr))
                    .data(expected_next_committee.as_ref())
                    .build(),
            ],
        );
    }

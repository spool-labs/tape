//! Chunk manifest for multi-track file storage.
//!
//! When a file exceeds the single-track size limit, the SDK splits it into
//! chunks stored across multiple tracks. A manifest track is written last,
//! recording the chunk layout so the file can be reassembled on read.
...
///
/// Clay(20,7) with 10 MiB slices gives a theoretical 70 MiB per track.
/// We use 64 MiB (a clean power of two) to leave headroom for stripe
/// padding and metadata suffixes.
pub const CHUNK_SIZE: usize = 64 * 1024 * 1024;

/// Describes a file stored across one or more chunk tracks on a single tape.
///
/// The manifest is always written as the last track. All chunk tracks must
/// exist before the manifest is written.
...
pub struct ChunkManifest {
    ...
    pub chunk_size: u64,
    /// File-level content key provided by the caller.
    pub key: Hash,
...
impl ChunkManifest {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>, FileError> {
        wincode::serialize(self).map_err(|error| FileError::Manifest(error.to_string()))
    }
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, FileError> {
        let manifest: Self = wincode::deserialize(bytes)
            .map_err(|error| FileError::Manifest(error.to_string()))?;
        manifest.validate()?;
        Ok(manifest)
    }
    ...
    fn validate(&self) -> Result<(), FileError> {
        if self.version != MANIFEST_VERSION {
            return Err(FileError::Manifest(format!(
        ...
        if self.chunks.is_empty() {
            return Err(FileError::Manifest("manifest has no chunks".into()));
        }
        ...
        for (i, entry) in self.chunks.iter().enumerate() {
            if entry.offset != expected_offset {
                return Err(FileError::Manifest(format!(
                    "chunk {i} offset mismatch: expected {expected_offset}, got {}",
                    entry.offset
                )));
            }

            if entry.size == 0 {
                return Err(FileError::Manifest(format!("chunk {i} has zero size")));
            }

            expected_offset = expected_offset
                .checked_add(entry.size)
                .ok_or_else(|| FileError::Manifest(format!("chunk {i} offset overflow")))?;
        }
...
mod tests {
    ...
    fn sample_manifest(chunk_count: usize) -> ChunkManifest {
        let chunks: Vec<ChunkEntry> = (0..chunk_count)
            .map(|i| ChunkEntry {
                track_number: TrackNumber(i as u64),
        ...
        ChunkManifest {
            ...
            total_size: chunk_count as u64 * CHUNK_SIZE as u64,
    ...
    fn smaller() {
        ...
        let manifest = ChunkManifest {
            ...
            chunks: vec![
                ChunkEntry { track_number: TrackNumber(0), offset: 0, size: CHUNK_SIZE as u64 },
                ChunkEntry { track_number: TrackNumber(1), offset: CHUNK_SIZE as u64, size: CHUNK_SIZE as u64 },
                ChunkEntry { track_number: TrackNumber(2), offset: 2 * CHUNK_SIZE as u64, size: CHUNK_SIZE as u64 },
                ChunkEntry { track_number: TrackNumber(3), offset: 3 * CHUNK_SIZE as u64, size: 1000 },
            ],
        };

        let bytes = manifest.to_bytes().expect("serialize manifest");
        let recovered = ChunkManifest::from_bytes(&bytes).expect("deserialize manifest");
        assert_eq!(recovered.chunks[3].size, 1000);
    }
}

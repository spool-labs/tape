...
pub struct Prediction {
    /// Epoch the previous run ended on.
    pub epoch: EpochNumber,
    /// Spool groups that epoch had.
...
impl Prediction {
    ...
    pub fn epoch_at(&self, now: u64) -> EpochNumber {
        ...
        EpochNumber(self.epoch.0.saturating_add(elapsed / self.epoch_duration.0))
    }
}
...
impl PeerRecord {
    ...
    pub fn reconcile_clock(&mut self, now: u64) {
        ...
        let is_future_quarantine =
            matches!(self.health, PeerHealth::Quarantined { until } if until > horizon);
        if is_future_quarantine || self.last_ok > horizon || self.last_fail > horizon {
            self.health = PeerHealth::Suspect;
...
impl BootstrapState {
    ...
    pub fn peer_mut(&mut self, node: Address) -> &mut PeerRecord {
        let index = match self.peers.iter().position(|record| record.node == node) {
            Some(index) => index,
            None => {

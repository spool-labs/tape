...
pub fn run_tui(
    snapshot: Arc<ArcSwap<ProdnetView>>,
...
fn render_frame(frame: &mut Frame<'_>, view: &ProdnetView, disconnected: bool) {
    let area = frame.area();
...
fn render_title_bar(frame: &mut Frame<'_>, area: Rect, view: &ProdnetView) {
    ...
    let left = format!(
        " PRODNET  Nodes:{}  Healthy:{}  Metrics:{}  C[{}/{}/{}]  {}",
        view.nodes.len(),
        healthy_nodes,
        metrics_nodes,
...
fn render_spool_grid(frame: &mut Frame<'_>, area: Rect, view: &ProdnetView) {
...
fn render_node_table(frame: &mut Frame<'_>, area: Rect, view: &ProdnetView) {
    let block = Block::default()
        .borders(Borders::ALL)
        .border_style(Style::default().fg(Color::DarkGray))
        .title(" Nodes ");

...
impl Orchestrator {
    pub fn new(config: ProdnetConfig) -> Result<Self> {
        std::fs::create_dir_all(&config.data_dir)
            .with_context(|| format!("create prodnet data dir: {}", config.data_dir.display()))?;

        let admin_path = config.data_dir.join("admin.json");
        let (admin, created) = load_or_create_admin_keypair(&admin_path)?;
        if created {
            info!(admin = %admin.pubkey(), path = %admin_path.display(), "created prodnet admin keypair");
        } else {
            info!(admin = %admin.pubkey(), path = %admin_path.display(), "loaded prodnet admin keypair");
        }
        ...
        let existing_nodes = processes
            .load_existing_nodes()
            .context("load existing prodnet nodes")?;
        if existing_nodes > 0 {
            info!(count = existing_nodes, "loaded existing prodnet node identities");
        }
    ...
    if path.exists() {
        return Ok((
            load_solana_keypair(path)
                .with_context(|| format!("load prodnet admin keypair: {}", path.display()))?,
            false,
        ));
    }

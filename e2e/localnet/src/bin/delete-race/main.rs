...
//!
//! Writes a track on a fresh tape, the way the TUI's upload key does, then
//! deletes it immediately and keeps retrying until it lands. What comes out is
//! how long the delete stays refused and what it is refused with, which is what
//! separates a race from peers that genuinely lost the track.
...
struct Cli {
    ...
    /// Epochs to reserve for, since a throwaway tape needs no more.
...
async fn main() -> Result<()> {
    ...
    for round in 1..=cli.rounds {
        ...
        // The clock starts the moment the write returns, which is where the
        // harness reaches for the delete.

...
fn render_node_table(frame: &mut Frame<'_>, area: Rect, view: &LocalnetView) {
    ...
    // One witness's record of everyone, which is what the record is: a local
    // view, not a network verdict. The lowest healthy id keeps it stable.
    let witness_grid: std::collections::HashMap<&str, &tape_observe_api::ChallengeRow> = view
        .nodes
        .iter()
        .filter(|node| node.healthy)
        .min_by_key(|node| node.local_id)
    ...
    let rows = nodes.into_iter().map(move |node| {
        ...
        let judged = witness_grid.get(node.node_address.as_str());
        let miss = judged

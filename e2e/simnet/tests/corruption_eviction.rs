...
///
/// The budget is set by how often the victim's spool is drawn, not by how
/// reliably the rot is caught: every draw after the rot is a miss, but the
/// record only advances when a round settles for that one spool, which is
/// closer to once every two epochs than once a round. A run that draws three
/// times finishes in half this; one that draws twice needs the rest.

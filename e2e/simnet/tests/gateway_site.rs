...
async fn site_serving_inner() {
    ...
    // The well-known namespace answers from the gateway itself: an ACME-style
    // probe on a served host gets a bare 404, never the site's 404 page.
    let response = client
        .get(format!("{base}/.well-known/acme-challenge/token"))
        .header("host", CUSTOM_DOMAIN)
        .send()
        .await
        .expect("request well-known path on custom domain");
    assert_eq!(response.status(), StatusCode::NOT_FOUND);
    let body = response.bytes().await.expect("read well-known body");
    assert!(body.is_empty(), "well-known must never serve tape content");
    eprintln!("gateway_site: well-known reserved");

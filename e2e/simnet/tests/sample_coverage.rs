//! Every coded slice a node holds is one its group can ask it about.
//!
//! The sample set is chain-derived, so it no longer follows what a node stored.
//! That only works if every path installing a coded track locally also records
//! it: replay does, and so must the snapshot build, whose tracks are most of
//! what an idle fleet holds. A slice with no row is one its owner can never
//! answer for, and it takes the miss.
//!
//! Unit tests cannot see this. It needs a fleet that has crossed epoch
//! boundaries, since snapshot tracks are what the boundary writes.
...
/// Epochs to cross, so the boundary has written snapshot tracks and the nodes
/// hold slices of them.
const STEADY_EPOCH: u64 = 3;

// a node can be asked about every coded slice it holds
#[test]
fn sample_coverage() {
    ...
    for index in 0..NODE_COUNT {
        ...
        for (spool, _) in store.iter_all_spools().expect("iter spools") {
            ...
            for (track, _) in store.iter_slice_sizes_by_spool(spool).expect("iter slices") {
                // Only coded tracks are sampled; an inline payload has no slices.
                let coded = matches!(

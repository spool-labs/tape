...
gateway-linux:
	$(call linux-cargo,.,--features metrics -p tape-gateway,tape-gateway)
...
# Static wasm bundle for the atlas globe kiosk (needs dioxus-cli: `dx`).
# Serve it with `tape-collector --www <bundle>`.
atlas-web:

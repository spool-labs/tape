...
mod tests {
    use super::*;
    use crate::{Stake, dhondt::DhondtSpooler};
    ...
    fn dhondt_counts(stakes: &[Stake], total: SpoolCount) -> Vec<SpoolCount> {
    ...
    fn fresh_large() {
        let n = 128;
        let members = make_members(n);
        let stakes: Vec<Stake> = (1..=n as Stake).map(|i| i * 1000).collect();
    ...
    fn fresh_uneven() {
        let n = 50;
        let members = make_members(n);
        let stakes: Vec<Stake> = (1..=n as Stake).map(|i| i * i * 100).collect();
        let counts = dhondt_counts(&stakes, SPOOL_COUNT as SpoolCount);
        let result = initial_assignment(&members, &counts).unwrap();
        verify_group_constraints(&result, n);
        verify_counts(&result, &counts);
    ...
    fn identity_stable() {
        let n = 30;
        let members = make_members(n);
        let stakes: Vec<Stake> = (1..=n as Stake).map(|i| i * 500).collect();
        let counts = dhondt_counts(&stakes, SPOOL_COUNT as SpoolCount);
    ...
    fn removal_disruption() {
        ...
        let stakes1: Vec<Stake> = (1..=n as Stake).map(|i| i * 300).collect();
        ...
        let stakes2: Vec<Stake> = stakes1[..30].to_vec();
    ...
    fn addition_disruption() {
        ...
        let stakes1: Vec<Stake> = (1..=n1 as Stake).map(|i| i * 500).collect();
        let counts1 = dhondt_counts(&stakes1, SPOOL_COUNT as SpoolCount);
        let epoch1 = initial_assignment(&members1, &counts1).unwrap();

        let n2 = 50;
        let members2 = make_members(n2);
        let stakes2: Vec<Stake> = (1..=n2 as Stake).map(|i| i * 500).collect();
        let counts2 = dhondt_counts(&stakes2, SPOOL_COUNT as SpoolCount);
    ...
    fn full_replace() {
        let n = 25;
        let members1 = make_members(n);
        let stakes: Vec<Stake> = vec![1000; n];
    ...
    fn epoch_chain() {
        ...
        for epoch in 0..5 {
            ...
            let new_members: Vec<NodeId> = (base as u64 + 6..=base as u64 + n as u64 + 5)
                .map(NodeId)
                .collect();
            let new_stakes: Vec<Stake> = (1..=n as Stake).map(|i| i * 1000).collect();
    ...
    fn deterministic_fresh() {
        let n = 50;
        let members = make_members(n);
        let stakes: Vec<Stake> = (1..=n as Stake).map(|i| i * 777).collect();
        let counts = dhondt_counts(&stakes, SPOOL_COUNT as SpoolCount);
    ...
    fn stake_stability() {
        ...
        let stakes1: Vec<Stake> = (1..=n as Stake).map(|i| i * 1000).collect();
        let counts1 = dhondt_counts(&stakes1, SPOOL_COUNT as SpoolCount);
    ...
    fn eviction() {
        ...
        let mut stakes1: Vec<Stake> = vec![100; n];
        for i in 0..5 {
            stakes1[i] = 10_000;
        }
        let counts1 = dhondt_counts(&stakes1, SPOOL_COUNT as SpoolCount);
        ...
        let mut stakes2: Vec<Stake> = vec![100; n];
        for i in (n - 5)..n {
            stakes2[i] = 10_000;
        }
        let counts2 = dhondt_counts(&stakes2, SPOOL_COUNT as SpoolCount);
    ...
    fn fresh_saturated() {
        for &n in &[20, 25, 50, 100] {
            ...
            let stakes: Vec<Stake> = (1..=n as Stake).map(|i| i * 100).collect();
    ...
    fn rebalance_reversed() {
        let n = 30;
        let m = make_members(n);
        let s1: Vec<Stake> = (1..=n as Stake).map(|i| i * 1000).collect();
        let c1 = dhondt_counts(&s1, SPOOL_COUNT as SpoolCount);
        let r1 = fresh(&m, &c1);

        let s2: Vec<Stake> = (1..=n as Stake).rev().map(|i| i * 1000).collect();
    ...
    fn epoch_large() {
        ...
        for epoch in 0..3 {
            ...
            let new_stakes: Vec<Stake> = (1..=n as Stake).map(|i| i * 100).collect();
            let new_counts = dhondt_counts(&new_stakes, SPOOL_COUNT as SpoolCount);
    ...
    fn stress_grow() {
        ...
        let stakes2: Vec<Stake> = (1..=n2 as Stake).map(|i| i * 100).collect();
    ...
    fn round_robin_identity() {
        let round_robin = |n: usize| -> Vec<SpoolMapping> {
            (0..SPOOL_COUNT)
                .map(|i| {
        ...
        for &n in &[20, 25, 50] {
            let spools = round_robin(n);
            let m = make_members(n);
            let per = (SPOOL_COUNT / n) as SpoolCount;
            let result = mig(&spools, &m, &m, &uniform(n, per));
            assert_eq!(result, spools, "identity failed for n={}", n);
        }
    }

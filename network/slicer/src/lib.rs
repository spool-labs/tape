#![allow(clippy::len_without_is_empty)]

pub mod consts;
pub mod errors;
pub mod types;
pub mod api;
pub mod basic;
pub mod codec;
pub mod striped;
pub mod rotated;
pub mod recovery;
pub mod nested;
pub mod lrc;
pub mod product;
pub mod msr;
pub mod merkle_helpers;
pub mod reed_solomon;
pub mod slice_index;

#[cfg(test)]
mod bench;

pub use consts::{MERKLE_HEIGHT, SLICE_COUNT, F, CODING_SLICES, DATA_SLICES};
pub use errors::{EncodeError, DecodeError};
pub use types::{Slice, Blob};
pub use api::Slicer;
pub use basic::BasicSlicer;
pub use codec::{DEFAULT_STRIPE_SIZE, ROTATION_STEP, STRIPE_SIZES, pick_stripe_size, SliceMetadata};
pub use striped::StripedSlicer;
pub use rotated::RotatedSlicer;
pub use recovery::{RecoveryCodec, RecoveryError, RecoveryLayout, RowEncodeError};
pub use nested::{NestedSlicer, NestedEncodeError, NestedDecodeError, EncodeResult, MIN_SEGMENT_SIZE};
pub use lrc::{LrcSlicer, LrcStats, LrcMetadata, LrcRepairError, LocalGroup, LOCAL_GROUP_SIZE, LOCAL_PARITIES, GLOBAL_PARITIES};
pub use product::{ProductSlicer, ProductStats, ProductMetadata, ProductRepairError, GRID_SIZE, DATA_DIM, PARITY_DIM, DATA_CELLS};
pub use msr::{MsrSlicer, MsrStats, MsrMetadata, ALPHA, D_HELPERS, BETA, REPAIR_DOWNLOAD, REPAIR_BANDWIDTH_FRACTION};
pub use merkle_helpers::{BlobMerkleTree, BlobMerkleRoot, build_blob_merkle_tree, blob_merkle_root};
pub use slice_index::SliceIndex;
pub use reed_solomon::MAX_SLICE_BYTES;

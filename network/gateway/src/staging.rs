...
impl StagingStore {
    ...
    ///
    /// Listing needs the same read-after-write treatment GET already has. An
    /// object that is staged but not yet resolvable on chain must still appear,
    /// or a client that writes and then lists does not see its own key, which
    /// is exactly what a ClickHouse S3 disk does for every part it writes.
    ///
...
mod tests {
    ...
    #[test]
    fn replace_does_not_leak_budget() {
        let store = StagingStore::new();

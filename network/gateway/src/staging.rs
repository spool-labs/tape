...
mod tests {
    ...
    use tape_store::types::PENDING_WRITE_CHUNK_BYTES;
    ...
    // an object wider than one stored chunk serves and drains its bytes whole
    #[tokio::test]
    async fn multi_chunk_object() {
        let staging = staging();
        let tape = Address::new([0x51; 32]);
        let data = pattern(PENDING_WRITE_CHUNK_BYTES * 2 + 7);

        put(&staging, tape, b"big.bin", &data).await;

        assert!(staging.has_bytes(tape, b"big.bin").expect("has bytes"));
        assert_eq!(staging.bytes(tape, b"big.bin").expect("bytes"), Some(data.clone()));
        assert_eq!(staging.queued_bytes(), data.len() as u64);
    }

    // a reopened queue recounts a multi-chunk payload's held bytes exactly
    #[tokio::test]
    async fn multi_chunk_recount() {
        let store = Arc::new(TapeStore::new(MemoryStore::new()));
        let tape = Address::new([0x52; 32]);
        let data = pattern(PENDING_WRITE_CHUNK_BYTES * 2 + 7);
        {
            let staging = StagingStore::try_new(store.clone()).expect("open queue");
            put(&staging, tape, b"big.bin", &data).await;
        ...
        assert_eq!(reopened.queued_bytes(), data.len() as u64);
        assert_eq!(reopened.tape_queued_bytes(tape), data.len() as u64);
        assert_eq!(reopened.bytes(tape, b"big.bin").expect("bytes"), Some(data));
    }
}

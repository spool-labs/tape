...
use tape_store::ops::ObjectListPage;
...
/// until the ingestor tails the slot and the track certifies, seconds later.
/// That is the read-after-write gap staging exists to close, and listing is the
...
fn merge_listing(
    ...
    if !staged.is_empty() {
        ...
        for (name, object) in staged {
            ...
            let folder = delimiter.and_then(|delim| {
                let rest = &name[prefix.len()..];
                find_subslice(rest, delim).map(|pos| name[..prefix.len() + pos + delim.len()].to_vec())
            });
            match folder {
                Some(folder) => {
    ...
    Blockchain: Rpc + 'static,
{
    ...
    state.staging.put(
        ...
        assembled.key.clone(),
        StagedObject::new(
            Bytes::from(assembled.data.clone()),
            assembled.content_type,
            etag,
            now_unix(),
        ),
...
mod merge_listing_tests {
    ...
    #[test]
    fn staged_key_folds_into_common_prefix() {
    ...
    #[test]
    fn existing_common_prefix_is_not_duplicated() {
    ...
    #[test]
    fn truncation_spans_objects_and_prefixes() {
    ...
    #[test]
    fn empty_staging_preserves_the_page() {

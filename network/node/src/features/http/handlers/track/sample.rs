...
pub async fn get_sample<Db: Store, Cluster: Api, Blockchain: Rpc>(
    ...
    let proof = usize::try_from(sub_leaf)
        .ok()
        .and_then(|sub_leaf| encoding.prove_sub_leaf(position, sub_leaf, &slice))
...
mod tests {
    ...
    use tape_core::erasure::{GROUP_SIZE, SUB_LEAF_BYTES, SUB_TREE_HEIGHT, sub_leaf_count};
    ...
    fn seed(ctx: &TestContext, spool: SpoolIndex) -> (Address, usize) {
        let (slices, encoding) = coded_track(PAYLOAD_BYTES, 0x2545_F491_4F6C_DD1D);

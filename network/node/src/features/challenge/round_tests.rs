...
impl Fixture {
    ...
    fn answer_from_leaf(&self, spool: SpoolIndex, leaf: Option<usize>) -> ProofOfAccess {
        ...
        let asked = expected_sample(&self.ctx, &round, spool, self.mine).expect("a sample");
...
async fn every_owner_is_asked_a_different_question() {
    ...
    let leaves: Vec<usize> = group_spools(&fixture.state, fixture.group)
        .into_iter()
        .filter_map(|spool| expected_sample(&fixture.ctx, &round, spool, fixture.mine))
...
async fn an_answer_to_an_easier_leaf_is_refused() {
    ...
    let fixture = fixture().await;
    let target = fixture.other();
    let asked = expected_sample(&fixture.ctx, &fixture.round(), target, fixture.mine).expect("sample");

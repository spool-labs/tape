...
struct RequestProbe {
    ...
    method: Method,
    start: Instant,
    /// Declared request body size. Counted from `Content-Length` rather than by
    /// wrapping the body, so a chunked upload with no declared length is not
    /// counted; every writer in this system sends a length.
    request_bytes: u64,
}

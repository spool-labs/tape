...
use tape_store::ops::{MultipartOps, PendingWriteOps};
use tape_store::types::{MultipartPart, PendingOp, PendingState, PendingWrite};
...
fn concurrent_parts_posix() {
    use reel::IoBackend;
    use reel_store::{node_config, NodeStoreOptions, ReelStore, Reserve, TAPE_COLUMNS};
    use tape_store::TapeStore;

    let dir = TempDir::new().expect("temp dir");
    let config = node_config(NodeStoreOptions {
        backend: IoBackend::Posix,
...
/// A queued object twice as wide as a small-volume segment, the size CompleteMultipartUpload assembles
const QUEUED_OBJECT_BYTES: usize = 64 * 1024 * 1024;

/// An object wider than a segment enqueues and reads back
#[test]
fn queued_object() {
    let dir = TempDir::new().expect("temp dir");
    let store = open_harness_store(dir.path()).expect("open");
    let tape = Address::new([7u8; 32]);
    let data = pattern(7, QUEUED_OBJECT_BYTES);
    let write = PendingWrite {
        seq: 1,
        op: PendingOp::Put {
            content_type: ContentType::Unknown,
            etag: Hash::default(),
            size: QUEUED_OBJECT_BYTES as u64,
            block_time: 0,
            prior: None,
        },
    ...
    block_on(store.put_pending_write(tape, b"big.bin", &write, Some(data.clone())))
        .expect("queue put");

    assert!(store.has_pending_write_data(tape, b"big.bin").expect("presence"));
    assert_eq!(store.get_pending_write_data(tape, b"big.bin").expect("read"), Some(data));
}

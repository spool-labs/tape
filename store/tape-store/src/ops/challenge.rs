...
/// Operations over this node's local per-spool challenge history.
///
/// Records stay separated by spool because certificates are issued per spool.
pub trait ChallengeOps {
    ...
    /// Forget one spool's record, once its owner no longer answers for it.
    fn delete_spool_record(&self, peer: Address, spool: SpoolIndex) -> Result<()>;
...
impl<S: Store> ChallengeOps for TapeStore<S> {
    ...
    fn delete_peer_record(&self, peer: Address) -> Result<()> {
        for (spool, _) in self.records_for_peer(peer)? {
            self.delete_spool_record(peer, spool)?;
        }
        Ok(())
    }

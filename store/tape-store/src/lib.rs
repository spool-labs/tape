...
/// column families live on the bulk tier
...
// Split-store constructors: a metadata store on the fast tier and a bulk store
// on the large tier, split by column family.
impl TapeStore<SplitStore> {
    ...
    /// tiered variant to place the bulk store on a separate device.
    ...
    pub fn open_primary_tiered<P: AsRef<Path>>(
        meta_dir: P,
        bulk_dir: P,
        compaction_rate_limit_mb_per_sec: u64,
    ) -> Result<Self, store::Error> {
    ...
    ///
    /// Passes the full column-family configs per tier: opening by column-family
    ...
    pub fn open_read_only<P: AsRef<Path>>(root: P) -> Result<Self, store::Error> {
        let root = root.as_ref();
        Self::open_read_only_tiered(root.join(config::META_SUBDIR), root.join(config::BULK_SUBDIR))
    }

    /// Open a read-only replica with the two tiers at explicit directories
    pub fn open_read_only_tiered<P: AsRef<Path>>(
        meta_dir: P,
        bulk_dir: P,
    ) -> Result<Self, store::Error> {
    ...
    /// Open a secondary for catch-up reads with the two tiers at explicit
    /// primary and secondary directories
    pub fn open_secondary_tiered<P: AsRef<Path>>(
        meta_primary: P,
        meta_secondary: P,
        bulk_primary: P,
        bulk_secondary: P,
...
mod tests {
    ...
    fn split_placement() {
        ...
        let store = TapeStore::open_primary_tiered(&meta_dir, &bulk_dir, 100).unwrap();

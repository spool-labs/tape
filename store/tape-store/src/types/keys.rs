...
/// Key for one spool's outcome in one round.
///
...
/// Key for one spool's record under one owner.
///
...
/// Key for the challenge sample set, by group then track.
///
...
/// Key for one queued S3 write: the tape address then the object key bytes, so one bucket scans together.
...
/// Key for one stored chunk of a queued object payload.
///
/// Format: `[tape 32 bytes][key_len BE 4 bytes][key bytes][chunk_index BE 4 bytes]`.
/// A payload is split across chunks so no stored value has to fit one store
/// segment. The leading tape keeps a whole-bucket scan working, and the length
/// prefix keeps one object's chunk scan out of the chunks of a key it is a
/// prefix of.
#[derive(Clone, Debug, Deserialize, Eq, Hash, PartialEq, Serialize)]
pub struct PendingWriteChunkKey {
    /// Bucket tape the write lands on.
    pub tape: Address,
    /// Object key, the on-chain track name.
    pub key: Vec<u8>,
    /// Zero-based position of this chunk within the payload.
    pub chunk_index: u32,
}

impl PendingWriteChunkKey {
    /// Encoded size of everything but the object key: tape, key length, chunk index.
    pub const FIXED_SIZE: usize = 40;

    /// Encoded size of the object prefix's fixed part: tape and key length.
    pub const PREFIX_FIXED_SIZE: usize = 36;

    /// Create a chunk key for `tape`, object `key` and `chunk_index`.
    pub fn new(tape: Address, key: Vec<u8>, chunk_index: u32) -> Self {
        Self {
            tape,
            key,
            chunk_index,
    ...
    /// Prefix bytes for scanning every chunk of one object
    pub fn object_prefix(tape: Address, key: &[u8]) -> Vec<u8> {
        let mut prefix = Vec::with_capacity(Self::PREFIX_FIXED_SIZE + key.len());
        prefix.extend_from_slice(&tape.to_bytes());
...
impl SchemaWrite for PendingWriteChunkKey {
    type Src = Self;

    fn size_of(src: &Self::Src) -> WriteResult<usize> {
        Ok(Self::FIXED_SIZE + src.key.len())
    }

    fn write(writer: &mut Writer, src: &Self::Src) -> WriteResult<()> {
        writer.write_exact(src.tape.as_ref())?;
        writer.write_exact(&(src.key.len() as u32).to_be_bytes())?;
        writer.write_exact(&src.key)?;
...
impl<'de> SchemaRead<'de> for PendingWriteChunkKey {
    type Dst = Self;

    fn read(reader: &mut Reader<'de>, dst: &mut MaybeUninit<PendingWriteChunkKey>) -> ReadResult<()> {
        // SAFETY: get_t reads a fixed 32-byte array for the Pod tape address; the
        // key's leading field is that known fixed width.
        let tape: [u8; 32] = unsafe { reader.get_t()? };
        // SAFETY: get_t reads a fixed 4-byte array for the Pod key length, which
        // follows the address at a known offset.
        let key_len_bytes: [u8; 4] = unsafe { reader.get_t()? };
        let key = reader.read_borrowed(u32::from_be_bytes(key_len_bytes) as usize)?.to_vec();
        // SAFETY: get_t reads a fixed 4-byte array for the Pod chunk_index field,
        // the key's trailing fixed width.
        let chunk_index_bytes: [u8; 4] = unsafe { reader.get_t()? };
        dst.write(PendingWriteChunkKey {
            tape: Address::from(tape),
            key,
...
mod tests {
    ...
    // a queued payload chunk key round-trips, key bytes and all
    #[test]
    fn queued_chunk_encoding() {
        let key = PendingWriteChunkKey::new(Address::new([5u8; 32]), b"logs/a.txt".to_vec(), 3);
        let bytes = wincode::serialize(&key).expect("serialize");
        assert_eq!(bytes.len(), PendingWriteChunkKey::FIXED_SIZE + b"logs/a.txt".len());
        let decoded: PendingWriteChunkKey = wincode::deserialize(&bytes).expect("deserialize");
        assert_eq!(key, decoded);
    }

    // an object's chunk prefix orders by index and never reaches a longer key
    #[test]
    fn queued_chunk_scoping() {
        let tape = Address::new([5u8; 32]);
        let prefix = PendingWriteChunkKey::object_prefix(tape, b"a");

        let c0 = wincode::serialize(&PendingWriteChunkKey::new(tape, b"a".to_vec(), 0))
            .expect("serialize");
        let c1 = wincode::serialize(&PendingWriteChunkKey::new(tape, b"a".to_vec(), 1))
            .expect("serialize");
        let longer = wincode::serialize(&PendingWriteChunkKey::new(tape, b"ab".to_vec(), 0))
            .expect("serialize");

        assert_eq!(&c0[..32], &PendingWriteKey::tape_prefix(tape));
        assert!(c0.starts_with(&prefix) && c1.starts_with(&prefix));
        assert!(c0 < c1, "chunks sort by index within an object");
        assert!(!longer.starts_with(&prefix), "a longer key is scanned separately");
    }
}

...
//!
//! Two write counters, because they answer different questions. `wchar` is what
//! the process passed to write syscalls, so it is the engine's own decision about
//! how much to put down. `write_bytes` is what the kernel attributed to the
//! process on its way to storage, accounted a page at a time, so a record smaller
//! than a page that lands on its own page still costs a page. Logical bytes are
//! key plus payload, the only figure the caller asked to store.
//!
...
impl Written {
    fn read() -> Written {
        ...
        Written { wchar: field("wchar:"), write_bytes: field("write_bytes:") }
    }

    fn since(&self, earlier: &Written) -> Written {
        Written {
...
///
/// The same fill the read bench uses, so an amplification here and a size there
/// are talking about the same payloads.
fn random(seed: usize, len: usize) -> Vec<u8> {
...
/// list, which is the shape of the `.md` files the product stores. Not real
/// English, but repetitive in the way real English is, so what lz4 finds here is
/// the kind of structure it would find in a real page.
fn markdown(seed: usize, len: usize) -> Vec<u8> {

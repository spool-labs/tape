...
//! shape, so a per-record figure is not rounding. Every number is a byte count.
//! There are no timings here at all, deliberately: the machine these rows are
//! taken on cannot produce comparable ones, and the question is what a slice
//! occupies and what writing it moved, not how long it took.
//!
...
fn sweep<Arm: SliceArm>(shape: &Shape) {
    ...
    for seed in 0..count {
        ...
        records.push((spool, Address::new_unique(), markdown(seed, shape.payload_len)));
    }
    ...
    let root = match std::env::var_os("TAPE_BENCH_KEEP") {
        Some(kept) => PathBuf::from(kept).join(format!("{}-slice-{}", Arm::NAME, shape.label)),
    ...
    println!(
        "{:>8} {:>8} {:>8} {:>9} {:>9} {:>9} {:>9} {:>11}",
        "arm", "shape", "records", "logical", "live", "actual", "wchar", "write_bytes",
    );
    println!(
        "{:>8} {:>8} {count:>8} {:>9.1} {:>9.1} {:>9.1} {:>9.1} {:>11.1}",
        Arm::NAME,
        shape.label,
        mib(logical),
        mib(live),

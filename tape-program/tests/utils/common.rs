...
pub fn get_committee_state(svm: &LiteSVM, committee: &Pubkey) -> Committee {
    let account = svm.get_account(committee).unwrap();
    *Committee::unpack_with_discriminator(&account.data).unwrap()
}

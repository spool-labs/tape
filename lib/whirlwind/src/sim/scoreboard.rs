...
mod tests {
    ...
    // the rule fires on the rate floor and on a run of misses
    #[test]
    fn eviction_rule() {
    ...
    // an observer never keeps a record about itself
    #[test]
    fn no_self_scores() {
    ...
    // a success resets the consecutive-miss run and keeps the totals
    #[test]
    fn record_run() {
        let mut board = Scoreboard::new();
        for _ in 0..3 {

...
mod tests {
    ...
    // relaying a proof defeats a spool that withholds it from some peers
    #[test]
    ...
    // scores survive a committee boundary instead of resetting
    #[test]
    ...
    // a round with no entropy block produces neither a certificate nor a gap
    #[test]
    fn void_rounds() {
    ...
    // an honest target gathers a real quorum and a free-rider never does
    #[test]
    fn real_certificate() {
        let data = ping_data();
    ...
    // the colluding free-rider never certifies and is voted out
    #[test]
    ...
    // a colluder bloc under the quorum cannot evict an honest node
    #[test]
    ...
    // every epoch keeps the committee at full size with distinct ids
    #[test]
    fn committee_size() {
        let config = fast_config();
        let report = simulate_epochs_with_costs(ping_data(), &config, test_costs()).unwrap();
    ...
    // a config that breaks the threshold relations is rejected
    #[test]

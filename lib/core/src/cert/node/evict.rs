...
/// Encode the target node's address as the eviction vote hash.
pub fn eviction_vote_hash(node: Address) -> Hash {
    Hash(node.to_bytes())
}

/// Recover the target node's address from an eviction vote hash.
pub fn eviction_vote_node(hash: Hash) -> Address {
    Address::new(hash.0)
}

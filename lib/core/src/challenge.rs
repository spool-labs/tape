...
//!
//! Both sides run this from the same inputs: the challenged owner to know what to
//! read, the challenger to know what it asked for. Nothing here touches storage,
//! so an owner cannot shrink its own sample set by dropping the data it lost.
//!
...
//! inspected. Whoever assembles the entries must produce the same order on both
//! sides, which `sort_entries` gives.
...
pub struct Sample {
    ...
    pub track: Address,
    /// Position within the group, which fixes which slice is read.
...
/// Put entries in the canonical order both sides must agree on.
...
pub fn sample_space(entries: &[SampleEntry]) -> u64 {
    entries
        .iter()
        .map(|entry| sub_leaf_count(entry.slice_len.as_u64() as usize) as u64)
        .sum()
}
...
pub fn draw(seed: &Hash, spool: SpoolIndex, entries: &[SampleEntry]) -> Option<Sample> {
    ...
    for entry in entries {
        let leaves = sub_leaf_count(entry.slice_len.as_u64() as usize) as u64;
        if index < leaves {
            return Some(Sample {
                track: entry.track,
                spool,
...
mod tests {
    ...
    fn an_empty_spool_draws_nothing() {
        assert!(draw(&seed_for(0), SpoolIndex(3), &[]).is_none());
        assert!(draw(&seed_for(0), SpoolIndex(3), &[entry(1, 0)]).is_none());
    }
    ...
    fn every_draw_lands_inside_its_track() {
        ...
        for round in 0..500u64 {
            let sample = draw(&seed_for(round), SpoolIndex(3), &entries).unwrap();
            let held = entries.iter().find(|e| e.track == sample.track).unwrap();
            assert!(sample.sub_leaf < sub_leaf_count(held.slice_len.as_u64() as usize));
            assert_eq!(sample.spool, SpoolIndex(3));
        }
    }
    ...
    fn a_trailing_partial_leaf_is_still_drawable() {
        ...
        for round in 0..200u64 {
            if draw(&seed_for(round), SpoolIndex(0), &[short]).unwrap().sub_leaf == 2 {
                saw_last = true;
            }
        }
        assert!(saw_last);
    ...
    fn the_order_of_entries_decides_the_answer() {
        // Both sides must sort the same way, so this pins that reordering the
        // input changes the draw and `sort_entries` is not optional.
        let mut forward = [entry(1, 10), entry(2, 10)];
        let mut reversed = [entry(2, 10), entry(1, 10)];
        let seed = seed_for(1);
        assert_ne!(
            draw(&seed, SpoolIndex(0), &forward),
            draw(&seed, SpoolIndex(0), &reversed)
        );

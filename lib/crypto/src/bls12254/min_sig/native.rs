...
/// Parses a field element only when its big-endian bytes are canonical.
///
/// `Fq::from_bigint` rejects integers at or above the field modulus instead of
/// reducing them, so one field element has exactly one accepted encoding.
fn fq(be: &[u8]) -> Option<Fq> {
    if be.len() != 32 {
        return None;
    }
    let mut limbs = [0u64; 4];
    for (limb, chunk) in limbs.iter_mut().rev().zip(be.chunks_exact(8)) {
...
pub fn g1_from_bytes(point: &G1Point) -> Result<G1Affine, BLSError> {
    let x = fq(&point.0[..32]).ok_or(BLSError::G1PointDecompressionError)?;
    let y = fq(&point.0[32..]).ok_or(BLSError::G1PointDecompressionError)?;
...
pub fn g2_from_bytes(point: &G2Point) -> Result<G2Affine, BLSError> {
    let x_real = fq(&point.0[32..64]).ok_or(BLSError::G2PointDecompressionError)?;
    let x_imaginary = fq(&point.0[..32]).ok_or(BLSError::G2PointDecompressionError)?;
    let y_real = fq(&point.0[96..128]).ok_or(BLSError::G2PointDecompressionError)?;
    let y_imaginary = fq(&point.0[64..96]).ok_or(BLSError::G2PointDecompressionError)?;
...
pub fn verify_signers<M: AsRef<[u8]>>(
    ...
    let keys = signers.iter().map(cached_g2).collect::<Result<Vec<_>, _>>()?;
...
mod tests {
    ...
    fn add_modulus(bytes: &mut [u8]) {
        let modulus = Fq::MODULUS.to_bytes_be();
        assert_eq!(bytes.len(), modulus.len());
        let mut carry = 0u16;
        for (byte, modulus_byte) in bytes.iter_mut().rev().zip(modulus.iter().rev()) {
            let sum = u16::from(*byte) + u16::from(*modulus_byte) + carry;
            *byte = sum as u8;
            carry = sum >> 8;
        }
        assert_eq!(carry, 0, "a field element plus the modulus fits in 256 bits");
    }

    #[test]
    fn noncanonical_coordinates_are_refused() {
        let (_, pubkeys) = committee(1);
        let canonical = pubkeys[0];
        let mut alternate = canonical;
        add_modulus(&mut alternate.0[..32]);

        // Reduction maps both byte strings to the same field element. Accepting
        // the alternate would therefore let one curve point evade byte-level
        // duplicate checks.
        assert_eq!(
            Fq::from_be_bytes_mod_order(&canonical.0[..32]),
            Fq::from_be_bytes_mod_order(&alternate.0[..32])
        );
        assert_ne!(canonical, alternate);

        assert!(g2_from_bytes(&canonical).is_ok());
        assert!(g2_from_bytes(&alternate).is_err());
        assert!(Committee::parse(&[canonical, alternate]).is_err());
        assert!(g2_from_bytes(&G2Point([0u8; 128])).is_err());
    }
}

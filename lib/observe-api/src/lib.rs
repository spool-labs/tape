...
/// Event names on the stream. Each carries the payload named below, so a client
/// dispatches on the event rather than sniffing the body.
...
/// One frame carrying recent history, so a chart opens full rather than
/// filling in over its whole window.
pub const EVENT_BACKFILL: &str = "backfill";

/// How often the node samples counters into a [`Tick`].
pub const TICK_PERIOD_MS: u64 = 250;

/// How often the node repeats the full [`Board`]. Everything on it that moves
/// faster than this is carried by [`Tick`] instead.
pub const BOARD_PERIOD_MS: u64 = 5_000;

/// How much recent history a connecting client is sent, and how coarsely.
/// One sample per second is enough to draw a window that is about to be
/// overwritten by live ticks anyway.
pub const BACKFILL_SPAN_MS: u64 = 90_000;
pub const BACKFILL_STEP_MS: u64 = 1_000;

/// Bumped when a frame's shape changes in a way an older client cannot read.
pub const STREAM_PROTOCOL: u32 = 1;

/// The opening frame, describing the producer and its cadences so the client can
/// size its interpolation window without hardcoding the node's constants.
...
/// Per-second rates and fast-moving gauges, sampled by the node every
/// [`TICK_PERIOD_MS`].
///
/// The node owns the differencing because it has the exact interval between
/// samples. Rates are per second regardless of the sampling period, so a client
/// plots them directly.
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
pub struct Tick {
    ...
    /// Bytes per second persisted and fetched, indexed by [`SPOOL_OPS`].
...
pub struct HttpStats {
    ...
    /// Request body bytes received. On the serving block this is what clients
    /// uploaded; on the peer-client block it is what this node sent to peers.
    #[serde(default)]

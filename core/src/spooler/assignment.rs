...
pub fn reassign_spools(
    ...
    // Merge current and next members into a unique set, and adjust the next_spool_counts to be
    ...
    // Verify total required spools equals number of current spools
    ...
    // Free current spools based on target counts in the unique set.
    let (free, remaining) = free_spools(current_spools, &target_counts);

    // Ensure freed spools exactly match the remaining demand
    ...
    // Assign free spools to members still needing spools
    let result = assign_spools(current_spools, &free, &remaining)?;

    // Remap spools into the the right index space
...
/// Free spools that are no longer needed, and decrement remaining demand for retained spools.
/// Keep spools with their current owner if possible; otherwise free them and reassign.
fn free_spools(
    spools: &[SpoolMapping],
...
/// Assign freed spools to nodes still needing spools.
fn assign_spools(
    current_spools: &[SpoolMapping],
    ...
    // For each member with unallocated spools, assign from freed spools.
    for member_index in 0..required_counts.len() {
        // Number of spools still needed for this member.
        let count = required_counts[member_index] as usize;
        for _ in 0..count {
            let spool_index = free_spools
                .pop()
                .expect("logic error: validated above that freed spools are sufficient") as usize;
...
/// Helper to create an initial spool map from spool counts, assigning spools contiguously.
pub fn to_spool_map(
...
mod tests {
    ...
    fn test_free_spools() {
        ...
        let (free, remaining) = free_spools(&current, &required);
        assert_eq!(free, vec![]);
    ...
    fn test_assign_spools() {
        ...
        let out = assign_spools(&current, &free, &required).unwrap();
    ...
    fn test_assign_spools_all() {
        ...
        let free = vec![0, 1, 2, 3, 4, 5]; // all spools freed
        let required = vec![2, 2, 2];
    ...
    fn test_single() {
        ...
        // Start with all spools owned by the single current member
        let spools = to_spool_map(&counts);

        let out = reassign_spools(&spools, &current, &next, &counts).unwrap();
    ...
    fn test_equal() {
        ...
        // Arbitrary initial: all spools assigned to the first current member
    ...
    fn test_reassign() {
        ...
        // Initial: each has 2 contiguous spools: [0,1]=n0, [2,3]=n1, [4,5]=n2, [6,7]=n3
        let initial_counts = vec![2, 2, 2, 2];
        let spools = to_spool_map(&initial_counts);
    ...
    fn test_reassign_reduce() {
        ...
        // Build initial spools contiguously
        let spools1 = to_spool_map(&counts1);

        // Shift to itself (no-op, but validates path)
        let out1 = reassign_spools(&spools1, &leaders1, &leaders1, &counts1).unwrap();
        ...
        // Reassign from previous spools/current to new next
        let out2 = reassign_spools(&out1, &leaders1, &leaders2, &counts2).unwrap();
    ...
    fn test_reassign_chain() {
        ...
        // Initial: each 2 contiguous spools
        ...
        let out1 = reassign_spools(&spools0, &current, &leaders1, &counts1).unwrap();
        ...
        // Original spools should still be included
        ...
        let out2 = reassign_spools(&out1, &current, &leaders2, &counts2).unwrap();
        ...
        let out3 = reassign_spools(&out2, &current, &leaders3, &counts3).unwrap();
    ...
    fn test_many() {
        ...
        // Verify total spools and correct number of nodes
        assert_eq!(total_count(&map2), SPOOLS);
        let unique_nodes: HashSet<NodeId> = map2.iter().cloned().collect();
        assert_eq!(unique_nodes.len(), 50, "Expected 50 nodes in updated spool map");

        // Verify spool counts match target_counts
        let spool_counts_map = counts_map(&leaders2, &counts2);
        for (node_id, &count) in &spool_counts_map {
            assert_eq!(
                spool_count(&map2, *node_id),
                count as usize,
                "spool count mismatch for node {:?}", node_id
            );
        }

        // Verify nodes not in updated_stakes have no spools
        for node_id in initial_stakes.keys() {
            if !updated_stakes.contains_key(node_id) {
                assert_eq!(
                    spool_count(&map2, *node_id),
                    0,
                    "Node {:?} should have no spools", node_id
                );
            }
        }

        // Print updated spool map
        println!("\nAfter reassignment:");
        print_table_header();
        for node_id in leaders2.iter() {
            let stake = updated_stakes.get(node_id).unwrap_or(&0);
            let spools_for_node = spool_list(&map2, *node_id);
            println!(
                "{:<8} | {:>12} | {:>6} | {:?}",
                format!("{:?}", node_id),
                stake,

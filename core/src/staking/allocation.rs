...
/// A fixed-size shard allocation structure.
#[repr(C)]
#[derive(Clone, Copy, Debug)]
pub struct ShardAllocation<const N: usize, const S: usize> {
    pub len: u16,               // number of nodes in the allocation (<= N and <= u16::MAX)
...
unsafe impl<const N: usize, const S: usize> Zeroable for ShardAllocation<N, S> {}
unsafe impl<const N: usize, const S: usize> Pod for ShardAllocation<N, S> {}

impl<const N: usize, const S: usize> ShardAllocation<N, S> {
    ...
    /// Create a new ShardAllocation from a map of NodeId -> shard count.
    pub fn new(node_shards: BTreeMap<NodeId, u16>) -> Self {
        ...
        Self::from_fixed(ids.as_slice(), &counts, &offsets, &shards)
    }

    /// Compute a new ShardAllocation from stake leaders and shard count.
    pub fn from(
        leaders: &StakingSet<N>,
        shard_count: u16,
    ) -> ShardAllocation<N, S> {
        let dist = stake_weighted_shard_counts(leaders, shard_count);
        ShardAllocation::new(dist)
    }

    /// Compute a new allocation from stake leaders and shard count, reusing
    /// the prior allocation to minimize shard movement.
    pub fn from_previous(
        previous: &ShardAllocation<N, S>,
        leaders: &StakingSet<N>,
        shard_count: u16,
    ) -> ShardAllocation<N, S> {
        let dist = stake_weighted_shard_counts(leaders, shard_count);
        if previous.size() == 0 {
            ...
            previous.apply_assignments(dist)
        }
    }

    /// Create a new allocation from fixed vectors.
    fn from_fixed(
        ids: &[NodeId], 
        counts: &[u16], 
        offsets: &[u16],
        ...
        for (i, &nid) in ids.iter().enumerate() {
            out.node_ids[i] = nid;
            out.shard_counts[i] = counts[i];
            out.shard_offsets[i] = offsets[i];
        }
    ...
    pub fn apply_assignments(&self, new_assignments: BTreeMap<NodeId, u16>) -> Self {
        ...
        Self::from_fixed(&ids_vec, &counts, &offsets, &shards)
    }
...
mod tests {
    ...
    type TestStakingSet = StakingSet<{ MAX_NODES }>;
    type TestAllocation = ShardAllocation<{ MAX_NODES }, { NUM_SHARDS }>;

    #[test]
    fn test_single_node() {
        let leaders = TestStakingSet::new(vec![(NodeId(10), 1_000_000)]);
        let allocation = TestAllocation::from(&leaders, 10);
        assert_eq!(allocation.size(), 1);
        assert_eq!(allocation.total_shards(), 10);
        assert_eq!(allocation.shards(&NodeId(10)).unwrap().len(), 10);
    }

    #[test]
    fn test_compute_even_distribution() {
        let leaders = TestStakingSet::new(vec![
            (NodeId(1), 1000), 
            (NodeId(2), 1000), 
            (NodeId(3), 1000)
        ]);

        let allocation = TestAllocation::from(&leaders, 6);
        assert_eq!(allocation.size(), 3);
        assert_eq!(allocation.shards(&NodeId(1)).unwrap().len(), 2);
        assert_eq!(allocation.shards(&NodeId(2)).unwrap().len(), 2);
        assert_eq!(allocation.shards(&NodeId(3)).unwrap().len(), 2);
    }
    ...
    fn test_compute_uneven_distribution() {
        ...
        let allocation = TestAllocation::from(&leaders, 10);
        assert_eq!(allocation.size(), 3);
        assert_eq!(allocation.shards(&NodeId(1)).unwrap().len(), 6);
        assert_eq!(allocation.shards(&NodeId(2)).unwrap().len(), 3);
        assert_eq!(allocation.shards(&NodeId(3)).unwrap().len(), 1);
    }
    ...
    fn test_consistent_shard_assignment() {
        ...
        let c1 = TestAllocation::new(init_map);
    ...
    fn test_complex_shard_assignment() {
        ...
        let c1 = TestAllocation::new([(n1, 2), (n2, 2), (n3, 2), (n4, 2), (n5, 2)].into());
    ...
    fn print_allocation<const N: usize, const S: usize>(
        leaders: &StakingSet<N>,
        allocation: &ShardAllocation<N, S>,
    ) {
        print_table_header();
        for i in 0..leaders.len as usize {
            let node = leaders.node_ids[i];
            let stake = leaders.stakes[i];
            let shards = allocation.shards(&node).unwrap_or(&[]);
            println!(
                "{:<8} | {:>12} | {:>6} | {:?}",
                node, stake, shards.len(), shards
            );
        }
        println!();
        println!(
    ...
    fn test_many_nodes() {
        ...
        let leaders = TestStakingSet::new(items);
        let allocation = TestAllocation::from(&leaders, 1000);

        print_allocation(&leaders, &allocation);

        println!("StakingSet size: {} bytes", TestStakingSet::size_bytes());
        println!("ShardAllocation size: {} bytes", TestAllocation::size_bytes());
    }
}

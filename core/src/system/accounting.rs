...
impl<const N: usize> StorageAccounting<N> {
    ...
    #[inline]
    pub fn get(&self, epoch: EpochNumber) -> Result<StorageUnits, AccountingError> {
        if epoch < self.now {
    ...
    pub fn reserve_capacity(
        ...
        for i in start_offset..end_offset {
            let entry = self.usage
                .get_mut(i)
                .ok_or(AccountingError::IndexOutOfBounds)?;
...
impl<const N: usize> RewardAccounting<N> {
    ...
    #[inline]
    pub fn get(&self, epoch: EpochNumber) -> Result<Coin::<TAPE>, AccountingError> {
        if epoch < self.now {
            return Err(AccountingError::EpochInPast);
        }
